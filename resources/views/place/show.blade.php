...
<style>
    ...
    .custom-cb:after {
        content: '';
        background-image: url('{{ asset('images/star-outline.svg') }}');
        display: block;
        width: 25px;
        height: 25px;
        background-size: 25px 25px;
        transition: background-image 0.3s ease-in-out, transform 0.1s linear;
    }

    .custom-cb.checke:after {
...
<section class="flex justify-center flex-wrap">
    ...
    <div class="w-full lg:w-2/3 my-10">
        ...
        <table class="m-auto table-auto">
            ...
            <td>Access:</td>
            <td>
                @for ($i = 0; $i < intval($average['access']); $i++)
                <img class="inline-block" width="15" src="{{ asset('/images/star.svg') }}" alt="">
            @endfor
                @if(intval($average['access']) < $average['access']) 
                    @if(($average['access'] - intval($average['access'])) >= 0.5)
                    <img class="inline-block" width="15" src="{{ asset('/images/star-half-outline.svg') }}" alt="">
                    @endif
                @endif
            </td>
            @endif
            ...
            <td>Location:</td>
           <td>
            @for ($i = 0; $i < intval($average['location']); $i++)
            <img class="inline-block" width="15" src="{{ asset('/images/star.svg') }}" alt="">
        @endfor
            @if(intval($average['location']) < $average['location']) 
                @if(($average['location'] - intval($average['location'])) >= 0.5)
                <img class="inline-block" width="15" src="{{ asset('/images/star-half-outline.svg') }}" alt="">
                @endif
            @endif
           </td>
            ...
            <td>Giftshops:</td>
            <td>
                @for ($i = 0; $i < intval($average['giftshops']); $i++)
                <img class="inline-block" width="15" src="{{ asset('/images/star.svg') }}" alt="">
            @endfor
                @if(intval($average['giftshops']) < $average['giftshops']) 
                    @if(($average['giftshops'] - intval($average['giftshops'])) >= 0.5)
                    <img class="inline-block" width="15" src="{{ asset('/images/star-half-outline.svg') }}" alt="">
                    @endif
                @endif
            </td>
            ...
            <td>Restaurants:</td>
            <td>
                @for ($i = 0; $i < intval($average['restaurants']); $i++)
                <img class="inline-block" width="15" src="{{ asset('/images/star.svg') }}" alt="">
            @endfor
                @if(intval($average['restaurants']) < $average['restaurants']) 
                    @if(($average['restaurants'] - intval($average['restaurants'])) >= 0.5)
                    <img class="inline-block" width="15" src="{{ asset('/images/star-half-outline.svg') }}" alt="">
                    @endif
                @endif
            </td>
    ...
    <div class="flex flex-wrap flex-col mt-20 bg-orange-100 py-20 items-center">
    ...
    <div class="flex flex-wrap bg-orange-600 justify-center">
        <h2 class="text-white text-center">Comments</h2>
    </div>
    <div>
        @if($comments)
        <div class="w-full flex items-center overflow-x-scroll py-12">
                ...
                <div class="inline-block">
                    <div class="flex flex-wrap rounded mx-5 shadow-lg p-5 comment-card">
                        <div class="w-full sm:w-3/12 py-5 sm:py-0 flex items-center">
                            @if ($c->user->avatar)
                            <img class="rounded-full" src="{{Storage::url('avatars/'.$c->user->id.'.jpg')}}" alt="{{ $c->user->name }} avatar Image">
                            @else
                        <div class="rounded-full text-white font-bold text-4xl flex items-center justify-center avatar bg-{{ $colors[array_rand($colors)] }}-500">{{substr($c->user->name,0,1)}}</div>
                            @endif
                        </div>
                        <div class="flex flex-col w-full sm:w-9/12">
                        <div><p class="pb-2"><strong>{{ $c->user->name }}</strong> on {{$c->updated_at}}</p><hr class="pb-2"></div>
                        <div>{{ $c->comment }}</div>
                        </div>
                    </div>
                </div>
